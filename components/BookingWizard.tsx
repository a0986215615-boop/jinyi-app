...
import { TIME_SLOTS, TAIWAN_HOLIDAYS } from '../constants';
...
export const BookingWizard: React.FC<BookingWizardProps> = ({ onBook }) => {
  ...
  const getAvailableTimeSlots = (dateStr: string) => {
    let slots = singleDoctor.availableSlots || TIME_SLOTS;
  ...
  const StepTime = () => {
    ...
    if (selectedDate) {
      const isHoliday = TAIWAN_HOLIDAYS.includes(selectedDate);
      const [y, m, d] = selectedDate.split('-').map(Number);
      const dateObj = new Date(y, m - 1, d);
      const isSunday = dateObj.getDay() === 0;
      isClosedDay = isHoliday || isSunday;
    }
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 pt-8 scrollbar-hide px-1">
            {availableDates.map((date) => {
              ...
              const isDateHoliday = TAIWAN_HOLIDAYS.includes(date);
              ...
              const isClosed = isDateHoliday || isSunday;
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-3 rounded-xl border transition-all flex flex-col items-center min-w-[85px] relative ${isFull || isClosed
                ...
                >
                  {isToday && !isFull && !isClosed && (
                    <span className={`absolute -top-2.5 left-1/2 transform -translate-x-1/2 text-[10px] px-2 py-0.5 rounded-full font-bold shadow-sm whitespace-nowrap ${selectedDate === date ? 'bg-white text-primary' : 'bg-red-500 text-white'
                      }`}>
                      今日
                    </span>
                  )}
                  ...
                  {isClosed && (
                    <span className="absolute -top-2.5 left-1/2 transform -translate-x-1/2 text-[10px] px-2 py-0.5 rounded-full font-bold shadow-sm whitespace-nowrap bg-orange-500 text-white">
                      休診
                    </span>
                  )}
                  ...
                  {activeTimeSlots.map((time) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-2 py-3 rounded-lg text-sm font-medium text-center transition-all border flex flex-col items-center justify-center gap-1 ${taken
        ...
        <div className="flex justify-end pt-4 mt-8 border-t border-slate-100">
          <button
            ...
            onClick={() => setStep(2)}
            className={`px-8 py-3 rounded-xl font-bold transition-all flex items-center gap-2 ${selectedDate && selectedTime
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-3 rounded-xl border outline-none transition-colors ${user
      ...
      <div className="text-center py-8 animate-fade-in">
        ...
        <div className="max-w-sm mx-auto mb-8">
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl border transition-all ${isReminderSet
